  ...
  return (
    <section id="projects" className="mx-auto max-w-6xl px-6 py-24 sm:py-32">
      <div ref={ref} className={`reveal ${visible ? 'visible' : ''}`}>
        <span className="eyebrow text-gold-400">Projects</span>
...
function ProjectCard({
  ...
  return (
    <div
      ref={ref}
      className={`reveal ${visible ? 'visible' : ''} group flex flex-col rounded-2xl border border-white/8 bg-white/[0.02] p-6 transition-all duration-300 hover:border-gold-500/25 hover:bg-gold-500/[0.03] hover:-translate-y-1`}
      style={{ transitionDelay: `${index * 100}ms` }}
    >
      {/* Icon */}
      <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl border border-gold-500/20 bg-gold-500/[0.06] transition-colors duration-300 group-hover:border-gold-500/40">
      ...
      <button
        ...
        className="mt-5 flex items-center gap-1.5 text-sm font-medium text-gold-300 transition-all duration-200 hover:gap-2.5 hover:text-gold-200"
      >
        View Case Study
